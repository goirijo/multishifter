...
{
    int stacks = 1;
    if (input_settings.contains("stacks"))
    {
        stacks = input_settings["stacks"];
    }
    return SlabSettings(input_settings["slab_unit"], stacks);
}

CleavageSettings CleavageSettings::from_json(const json& input_settings) { return CleavageSettings(input_settings["cleave"]); }
